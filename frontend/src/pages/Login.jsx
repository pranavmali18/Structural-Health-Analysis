...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    login({ email, name: email.split('@')[0], role: 'ENGINEER' });
    navigate('/dashboard');
  };
    ...
    <div className="max-w-md mx-auto py-12 space-y-6">
      ...
      <div className="glass-panel p-8 rounded-3xl border border-slate-800 shadow-2xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-xl font-bold text-slate-100">Engineer Portal Login</h2>
          <p className="text-xs text-slate-400 font-mono">Access SHM Monitoring Console</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4 text-xs font-mono">
          <div>
            <label className="block text-slate-400 mb-1">Engineering Email</label>
        ...
        <p className="text-center text-xs text-slate-400">
          Don't have an account? <Link to="/register" className="text-cyan-400 hover:underline">Register New User</Link>
        </p>
      </div>
    </div>
  );
